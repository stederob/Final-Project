...
light_brown = COLOR([194.0/255, 115.0/255, 51.0/255])
brown = COLOR([101.0/255, 67.0/255, 33.0/255])
...
def table_top(c0,c1,c00,c11,w,h,s):
	...
	bez3 = T([3])([h])(bez1);
	bez4 = T([3])([h])(bez2);
	bez5 = light_brown(T([3])([h-0.1])(MAP(BEZIER(S2)([c1,c11]))(dom2D)));
...
def table(n,w,d,s):
	...
	for i in range(n):
		...
		p0 = [[0.3+i*s,0+i*s,0], [0.3+i*s,0.3+i*s,0]]
		p1 = [[3.4-i*s,0+i*s,0],[4-i*s,0.6+i*s,0]]
		p2 = [[3.4-i*s,0.3+i*s,0],[3.7-i*s,0.6+i*s,0]]
		...
		p00 = [[0.3 + i*s,d-(0+i*s),0],[0.3+i*s,d-(0.3+i*s),0]]
		p11 = [[3.4-i*s,d-(0+i*s),0],[4-i*s,d-(0.6+i*s),0]]
		p22 = [[3.4-i*s,d-(0.3+i*s),0],[3.7-i*s,d-(0.6+i*s),0]]
		p3 = [[0.3+2*i*s,0+2*i*s,0+s], [0.3+2*i*s,0.3+2*i*s,0+s]]
		p4 = [[0.3 + 2*i*s,(d-(0+i*s))+i*s,0+s],[0.3+i*s,(d-(0.3+i*s))+i*s,0+s]]
...
spheres_r= T([1,2,3])([1.4,-0.2,7.1])(spheres(0.2,4))
spheres_l = T([1,2,3])([1.4,6.3,7.1])(spheres(0.2,4))
sp = STRUCT([spheres_l,spheres_r])
table = STRUCT([t[0],T([1])([-1.5])(t[1]),T([1])([-2.5])(t[2]),T([1])([-4])(t[3]),sp])
VIEW(table)
